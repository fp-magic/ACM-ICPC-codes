...
int p[400005];
vector<int>b[400005];
pair<int,int>c[800005],d[800005];
int e[800005];
int flag[200005][4];
int vit[400005];
...
bool ok(int dis,int tot){
    ...
    for(i=0;i<tot;++i){
        ...
        while(d[j].first<c[i].first-dis){
            if(e[d[j].second]>dis){
                j++;
                continue;
            }
            if(flag[d[j].second/4][(d[j].second%4)^1]&&((flag[d[j].second/4][((d[j].second%4|1)+1)%4]+flag[d[j].second/4][((d[j].second%4|1)+2)%4]<2))){
                now--;
            }
            //printf("-%d",s.second);
            flag[d[j].second/4][d[j].second%4]=0;
            j++;
        }
        //if(dis<=5)printf("=%d %d\n",c[i].second,now);
        ...
        for(i=1;i<=n;++i)
            if(b[i].size()){
                ...
                if(t==0){
                    ...
                    d[tot+1].first=d[tot].first;
                    d[tot+1].second=tot+1;
                    d[tot+3].first=d[tot+2].first;
                    d[tot+3].second=tot+3;
                    e[tot+1]=c[tot].first-d[tot].first;
                    e[tot+3]=c[tot+2].first-d[tot+2].first;
                }else{
                    c[tot+1].first=qr;
                    c[tot+1].second=tot+1;
                    c[tot+3].first=pr;
